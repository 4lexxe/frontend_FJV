    ...
    if (file) {
      ...
      return this.http
        ...
        .pipe(
          map((response) => {
            console.log(
              'Respuesta del backend (actualizar con foto):',
              response
            );
            const persona = response.persona || response.data || response;
            return this.mapPersonaToAfiliado(persona);
...
>>>>>>> feature/admin-dashboard-metrics
  ...
  uploadFoto(file: File): Observable<string> {
    ...
    return this.http
      .post<{ url: string }>(`${this.apiUrl}/upload-foto`, formData)
      .pipe(map((response) => response.url));
  }

  renovarLicencia(idPersona: number): Observable<Afiliado> {
    return this.http
      .put<any>(`${this.apiUrl}/personas/${idPersona}/renovar`, {})
      .pipe(
        map((response) => {
  ...
  actualizarEstadoLicencias(): Observable<any> {
    return this.http
      ...
      .pipe(
        catchError((error) => {
          console.log(
            'Error al actualizar estados de licencias (usando método GET):',
            error
          );
