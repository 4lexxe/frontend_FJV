...
export class NuevoClubPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (clubId) {
      this.clubService.obtenerClubPorId(+clubId).subscribe({
        next: (club) => this.clubParaEditar = club,
        error: (err) => {
  ...
  onGuardarClub(club: Club): void {
    ...
    const operation = club.idClub 
      ? this.clubService.actualizarClub(club.idClub, club) 
      : this.clubService.agregarClub(club);
